...
func (self *AlgX) dfs(index int) {
	////fmt.Println("dfs")
	//llog(-1, "dfs index %d len(%d) bombCards(%v)  flushCards(%v)\n", index, len(bombCards), bombCards, flushCards)

	if index == self.bombLen { //todo check 进入到这里所有的可能产生冲突的牌状态已经定了，后边可根据state的指引分别求顺子和炸弹最大即可

		bps := self.searchBomb()
		//self.searchBomb()
		//fmt.Println(self.bombTakeAheadMaxScoreProjects)
		self.searchFlush()
	...
	case 3:
		...
		case 3: //在无joker的情况下只有两种情况，在有joker的情况下又多了3种情况
			...
			//cardVal := bombCards[index][0].first
			for i := 0; i < self.bombCardsLen[index]; i++ {
				c := thisBombCards[i]
				cnt := thisBombColorMap.colors[c.Second]
				self.states[CARD_OFFSET(&c)][0] = cnt //给顺子
			...
			if self.jokerLeft > 0 { //任选两个组成炸弹，共三种情况
				...
				if self.jokerLeft > 0 {
					self.OneJokerMiddleNode(index) //把本层预处理之后再回到本层
				}
	...
	case 4:
		...
		case 5: //共6种
			...
			if self.jokerLeft > 0 { //任选两个组成炸弹去跟joker组成炸弹共6种情况
				...
				//llog(-1, "thisBombColorMap before jokerLeft %d OneJokerMiddleNode %v val %d\n", self.jokerLeft, thisBombColorMap, val)
...
//func (self *AlgX) dfsold(index int) {
//	////fmt.Println("dfs")
//	//llog(-1, "dfs index %d len(%d) bombCards(%v)  flushCards(%v)\n", index, len(bombCards), bombCards, flushCards)
//
//	if index == len(self.bombCards) { //todo check 进入到这里所有的可能产生冲突的牌状态已经定了，后边可根据state的指引分别求顺子和炸弹最大即可
//
//		bps := self.searchBomb()
//		//fmt.Println(self.bombTakeAheadMaxScoreProjects)
//		self.searchFlush()
//		//fmt.Println(*bps, *fps)
//
//		//fmt.Println("bps, fps:", bps, fps)
//
//		//var fpsScore, bpsScore, fpsLen, bpsLen int16
//		//
//		//if fps != nil {
//		//	fpsScore = fps.score
//		//	fpsLen = fps.len
...
//
//		for i := 0; i < self.tmpMaxScoreProjects.projLen; i++ {
//			if len(self.tmpMaxScoreProjects.projs[i].tagIndex) > 1 {
//				if !self.tmpMaxScoreProjects.FlushTrySplit(i) {
//					twoJokerInOneFlush = true
//					break
//				}
...
//		}
//
//		//for i := range self.tmpMaxScoreProjects.projs {
//		//	if len(self.tmpMaxScoreProjects.projs[i].tagIndex) > 1 {
//		//		if !self.tmpMaxScoreProjects.FlushTrySplit(i) {
//		//			twoJokerInOneFlush = true
//		//			break
...
//		//}
//
//		if bps.score+self.tmpMaxScoreProjects.score+self.bombTakeAheadMaxScoreProjects.score > self.maxScoreProjects.score && !twoJokerInOneFlush {
//
//			self.maxScoreProjects.updateUse(&self.tmpMaxScoreProjects)
//
//			if bps != nil && bps.score > 0 {
//				self.maxScoreProjects.merge(bps)
//			}
//
//			if self.bombTakeAheadMaxScoreProjects.score > 0 {
...
//		//if flps != nil {
//		for i := 0; i < self.tmpMaxLenProjects.projLen; i++ {
//			if len(self.tmpMaxLenProjects.projs[i].tagIndex) > 1 {
//				if !self.tmpMaxLenProjects.FlushTrySplit(i) {
//					twoJokerInOneFlush = true
//					break
//				}
//			}
//		}
//		//for i := range self.tmpMaxLenProjects.projs {
//		//	if len(self.tmpMaxLenProjects.projs[i].tagIndex) > 1 {
//		//		if !self.tmpMaxLenProjects.FlushTrySplit(i) {
//		//			twoJokerInOneFlush = true
//		//			break
...
//		//}
//
//		if bps.len+self.tmpMaxLenProjects.len+self.bombTakeAheadMaxScoreProjects.len > self.maxLenProjects.len && !twoJokerInOneFlush {
//
//			self.maxLenProjects.updateUse(&self.tmpMaxLenProjects)
//
//			if bps != nil && bps.len > 0 {
//				self.maxLenProjects.merge(bps)
//			}
//
//			if self.bombTakeAheadMaxScoreProjects.score > 0 {
