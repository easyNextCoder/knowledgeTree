...
func (engine *Engine) logSQLQueryTime(sqlStr string, args []interface{}, executionBlock func() (*core.Stmt, *core.Rows, error)) (*core.Stmt, *core.Rows, error) {
	if engine.showSQL && engine.showExecTime {
		...
		if execDuration > time.Second {
			if len(args) > 0 {
				if len(args) > 10 {
					args = args[:10]
				}
				engine.logger.Infof("[sql] [start:%d] [took:%s] %s [args] %v", UnixMilli(b4ExecTime), execDuration, sqlStr, args)
			} else {
				engine.logger.Infof("[sql] [start:%d] [took:%s] %s", UnixMilli(b4ExecTime), execDuration, sqlStr)
			}
		}
