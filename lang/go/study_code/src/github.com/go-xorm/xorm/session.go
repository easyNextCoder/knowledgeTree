...
// LastSQL returns last query information
func (session *Session) GetLastSQL() string {
	return fmt.Sprintf("sql=%s, args=%v", session.lastSQL, session.lastSQLArgs)
}

func (session *Session) GetLastSQLNew() string {
	//session.lastSQL, session.lastSQLArgs
	query := session.lastSQL
	args := session.lastSQLArgs

	for _, v := range args {
		str := ""
		switch val := v.(type) {
		case time.Time:
			str = fmt.Sprintf(`"%s"`, val.Format("2006-01-02 15:04:05"))
		case string:
