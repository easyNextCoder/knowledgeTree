...
解决方案：内部遍历的逻辑永远不变，只是改变最终的输出逻辑（另一种别人的思路好像是遍历出来之后reverse一下，正确性未知？待验证）
...
* 自己的两种解法代码：
		...
		public:
		    ...
		    TreeNode* findLCA(TreeNode* root, TreeNode* p, TreeNode* q)
		    {
		        ...
		        auto rval1 = findLCA(root->left, p, q);
		        if(rval1 != NULL)return rval1;
		        else if(findNum == 2 && canBeCA == 1){
		            return root;
		        }
		
		        auto rval2 = findLCA(root->right, p, q);
		        if(rval2 != NULL)return rval2;
		        else if(findNum == 2 && canBeCA == 1){
		            return root;
		        }
		    ...
		    TreeNode* lowestCommonAncestor(TreeNode* root, TreeNode* p, TreeNode* q) {
		        ...
		        auto rval = findLCA(root, p, q);
		        findNum = 0;
		        return rval;
		    }
		};
		int Solution:: findNum = 0;
		*/
		//自己在使用方法2做一下
		class Solution {
		public:
		    ...
		    TreeNode* lowestCommonAncestor(TreeNode* root, TreeNode* p, TreeNode* q) {
		        ...
		        auto rval = lowestCommonAncestor(root->left, p, q);
		        auto rval1 = lowestCommonAncestor(root->right, p, q);
...
* 二叉搜索树的重要性质
	* 根节点大于所有的左边子树节点，小于所有右边子树节点（大于左子树最大的，小于右子树最大的）存在相等的情况都是不对的。
	* 二叉树的中序遍历一定是一个递增的序列
...
* 题目：力扣：判断一棵树是不是二叉搜索树
	...
	* 这个题两种解法：分别对应着二叉搜索树的两个性质(以下是一种解法)
	>https://leetcode-cn.com/problems/legal-binary-search-tree-lcci/solution/shuang-bai-by-1ujin/